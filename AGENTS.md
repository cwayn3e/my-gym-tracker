...
### 4. История багов и фиксов
- **SyntaxError `}` на строке 104** — дублирующийся блок регистрации SW после rebase. Удалён.
- **Пропажа функций sync/export/import** — при разрешении merge-конфликта удалились ~300 строк (SUPABASE_URL, syncToCloud, loginAccount, exportData, importData). Восстановлены из коммита `a78de71`.
- **Кодировка файла** — пересохранён в UTF-8 (был Windows-1251 после `git show`).
- **Toast о завершении** — `"Тренировка завершена! ✨"` → `"Тренировка завершена и сохранена! ✅"`

### 5. Известные проблемы
- Два набора файлов: корень (`script.js`) и `public/script.js` — нужно поддерживать синхронизацию
- `public/script.js` — старая копия, без последних правок чата
